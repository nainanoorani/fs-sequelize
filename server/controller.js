...
module.exports = {
    ...
    getAllClients: (req,res) => {
        sequelize.query(`SELECT * FROM cc_users AS u JOIN cc_clients AS c ON u.user_id = c.user_id`)
            .then(dbRes => 
                //kitchen sends back to front-end. data always saved at index 0
                res.status(200).send(dbRes[0])) 
            .catch(err => console.log(err))
    },
    ...
    completeAppointment: (req,res) =>{
        let {apptId} = req.body;
        sequelize.query(`UPDATE cc_appointments a SET completed = TRUE 
